...
import { 
  ...
  ChevronRight, Radio, BarChart2, Search, ShieldCheck, Wand2, Target, BookOpen, Globe
} from 'lucide-react';
import { 
  LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer 
} from 'recharts';
import { Button, Card, RichTextRenderer, Input } from './Shared';
import { Season, Episode, ArcPoint, ViewState } from '../types';
...
export const SeasonPlanner = ({ seasons, setSeasons, selectedEpisode, setSelectedEpisode, onNavigate }: SeasonPlannerProps) => {
  ...
  const handleEmergencyEpisode = async (seasonId: string) => {
    setIsGenerating(true);
    const data = await geminiService.generateEmergencyEpisode();
    if (data) {
        const newEp: Episode = {
            id: `e-${Date.now()}`,
            title: `🚨 ${data.title}`,
  ...
  // Detail View (Episode Planning Page)
    ...
    <div className="h-full flex flex-col space-y-6">
      <div className="flex justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
              {/* Studio Toggle in Main View */}
              <Button variant="secondary" icon={Radio} className="text-red-500 hover:text-red-600 bg-red-500/5 hover:bg-red-500/10" onClick={() => onNavigate(ViewState.STUDIO)}>Studio Mode</Button>
               ...
               <div key={season.id} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                       ...
                       <button 
                         onClick={() => handleEmergencyEpisode(season.id)}
                         disabled={isGenerating}
                         className="flex items-center justify-center gap-2 p-4 border-2 border-dashed border-red-500/30 rounded-xl text-red-400 hover:bg-red-500/5 transition-all"
                       >
